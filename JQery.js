	...
	в то время как ( пока ) {
		...
		если ( ! соответствует || ( соответствует = rcomma.exec ( soFar )) ) {
			если ( совпадение ) {
				...
				soFar = soFar.slice(match[ 0 ].length ) || слишком далеко;
			}
			groups.push((токены = []));
		}
...
функция acceptValue (значение, разрешение, отклонение, noValue) {
	...
	// Отложено#тогда для условного подавления отклонения.
	} поймать ( значение ) {
...
jQuery.extend( {
	...
	// срабатывает событие готовности. См. #6781
	готовПодождите: 1,
...
// Microsoft забыла изменить свой префикс поставщика (#9572)
...
// Поддержка: только Android 4.0 
	- 4.3
	// Состояние проверки потеряно, если установлено имя (#11217)
	// Поддержка: веб-приложения Windows (WWA)
	// `имя` и `тип` должны использовать .setAttribute для WWA (#14901)
...
функция getAll (контекст, тег) {
	...
	// Используйте typeof, чтобы избежать вызова метода без аргументов для хост-объектов (#15151)
	вар рет;
...
jQuery.each({фокус: "фокус", размытие: "фокусаут"}, функция(тип, делегатТип) {
	jQuery.event.special [тип] = {
		...
		// Подавить собственный фокус или размытие, так как он уже запущен
		// в кредитном плече.
		_по умолчанию: функция () {
			вернуть истину;
		},
...
jQuery.fn.extend( {
	...
	off: функция (типы, селектор, fn) {
		var handleObj, тип;
		если ( типы && types.preventDefault && types.handleObj ) {
...
функция domManip (коллекция, аргументы, обратный вызов, игнорируется) {
	...
	если ( л ) {
		...
		если ( первый || игнорируется ) {
			...
			// неправильно очищается в определенных ситуациях (#8070).
			для ( ; я < 1; я ++ ) {
				узел = фрагмент;
